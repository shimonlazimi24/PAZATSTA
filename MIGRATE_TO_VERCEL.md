...
| Name | Value |
|------|-------|
| `DATABASE_URL` | **חובה** – לריצה. Vercel לא מגיע ל-Direct (db.xxx). השתמש ב-**Session mode** pooler (אותו ערך כמו DIRECT_URL) |
| `DIRECT_URL` | **חובה** – למיגרציות. אותו Session mode pooler. Supabase → Connect → "Session mode" |
